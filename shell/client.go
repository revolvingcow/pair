...
func Client(room, privateKey, address, port string) {
	...
	if err = cmd.Run(); err == exec.ErrNotFound {
		log.Println("Spawning an SSH client")
		spawnLocalClient(room, privateKey, address, forward, username)
	}

	log.Println("Connection closed")
}

func spawnLocalClient(room, privateKey, address, port, username string) {
